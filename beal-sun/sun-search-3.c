...
int main(int argc, char** argv){
    ...
    const int KB = 27, KHI = 2500;   /* defer k>2500 to phase 2 */          /* bitmap to k<=27, MR64 to 63, GMP beyond */
    LIM = N + (1ULL<<KB) + 64;
    sieve(); init_pr();
    mpz_t t; mpz_init(t);
    ...
    for(u64 n=nlo;n<=nhi;n++){
        int found=0, k;
        for(k=(n%2==0)?1:2; k<=KB && k<(int)n; k+=2)
        ...
        if(!found){
            ...
            if(!found){ unresolved++; printf("DEFER %llu\n", n); }
        }
        if(found && k>maxk){ maxk=k; maxn=n; fprintf(stderr,"new hardest n=%llu k=%d\n", n, k); }
        if(((n-nlo)&0xFFFFFF)==0) fprintf(stderr,"  n=%llu (%.0f%%)\r", n, 100.0*(n-nlo)/(nhi-nlo));
    }
